...
class SummaryCandidateSelector:
    def __init__(self) -> None:
        self.strong_signal_patterns = {
            "issue": [r"\bbug\b", r"问题", r"异常", r"报错", r"error"],
            "action_item": [r"\bTODO\b", r"待办", r"需要", r"请.*(完成|处理|跟进)", r"action"],
            "decision": [r"决定", r"结论", r"拍板", r"final decision"],
    ...
    def process(self, message: NormalizedMessage) -> ImportanceAnnotation:
        ...
        reason = "matched: " + ", ".join(sorted(set(matched_signals))) if matched_signals else "no strong signal"
        return ImportanceAnnotation(
            status=AnnotationStatus.DONE,
            score=score,
            level=level,

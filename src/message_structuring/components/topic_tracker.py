...
class TopicTracker:
    ...
    KEYWORD_ALIASES: dict[str, tuple[str, ...]] = {
        "login": ("登录", "登陆", "login"),
        "api": ("api", "接口", "gateway", "网关"),
        "incident": ("故障", "异常", "报错", "incident", "error", "500"),
        "rollback": ("回滚", "rollback"),
        "token": ("token", "鉴权", "认证"),
        "fix": ("修复", "恢复", "fix", "hotfix"),
        "rag": ("rag", "检索", "召回", "向量", "embedding", "chunk"),
        "summary": ("summary", "总结", "摘要", "归纳"),
        "migration": ("迁移", "migration", "cutover"),
        "redis": ("redis", "缓存"),
        "dependency": ("依赖", "dependency", "remove", "移除", "替换"),
        "prd": ("prd", "文档", "方案"),
        "ppt": ("ppt", "汇报"),
        "meeting": ("会议", "meeting", "同步", "周会"),
        "schedule": ("明天", "下午", "今天", "本周", "下周", "deadline", "ddl"),
        "task": ("任务", "待办", "action", "处理", "跟进"),
        "release": ("上线", "发布", "release"),
    }
        ...
        if not text:
            return (
                TopicAnnotation(
                    ...
                    similarity=0.0,
                    reason="empty message skipped",
                ),
                None,
            )

        should_skip, skip_reason = self._should_skip_message(message)
        if should_skip:
            return (
                TopicAnnotation(
                    status=AnnotationStatus.SKIPPED,
        ...
        if topic_from_conversation is not None:
            ...
            return (
                TopicAnnotation(
                    ...
                    similarity=0.97,
                    reason="inherited by thread/root",
                ),
                updated,
            )
            ...
            if issue_topic is not None:
                ...
                return (
                    TopicAnnotation(
                        ...
                        topic_title=updated.topic_title,
                        similarity=0.84,
                        reason="attached to recent issue topic",
                    ),
                    updated,
                )
    ...
    def _should_skip_message(self, message: NormalizedMessage) -> tuple[bool, str]:

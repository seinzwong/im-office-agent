...
如果进入决赛，我们会重点把项目从“单次群聊总结”升级为“持续理解群聊协作状态”的 IM Agent。

### 1. 结构化消息树

当前版本主要按时间窗拉取群消息并生成总结。决赛阶段计划引入 **结构化消息树**：系统持续监听群聊消息，从自然对话中浮现出 `task` 与 `topic`，并按照树状结构组织上下文。
...
这棵树会把原本按时间排列的聊天记录，整理成按主题、任务、决策和风险组织的上下文。后续送入 Agent 时，不再只是输入一大段聊天文本，而是输入更清晰的协作状态，从而提高总结、计划生成和复杂交付物生成的稳定性。

### 2. 多 Agent 编排

当前版本已经有 Gateway 调用外部 Agent 的协议基础。决赛阶段计划在 Agent 层加入更明确的多 Agent 编排，让不同 Agent 分别处理不同类型的工作：

- **Message Structuring Agent**：从群聊中维护 task/topic 消息树。
- **Summary Agent**：基于消息树生成纪要、行动项和风险列表。
- **Document Agent**：生成适合飞书文档的结构化内容。
- **Board Agent**：生成适合画板表达的流程、模块和关系图。
- **Slide Agent**：生成汇报型 SlideDraft 和 PPTX。
- **Review Agent**：检查产物是否忠实于原始消息，减少幻觉和遗漏。

目标是让系统从“生成一个总结”进化为“围绕一个项目持续产出复杂交付物”：例如项目周报、方案文档、任务看板、复盘材料和答辩 PPT 都可以从同一棵结构化消息树中生成，并由人工做最终确认。

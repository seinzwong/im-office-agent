...
1. 终端 A：Mock Agents  
   `cd mocks/agents && pip install -r requirements.txt && AGENTS_M2M_TOKEN=dev-m2m-secret uvicorn main:app --port 18080`
2. 终端 B：Gateway（开发跳过 lark 写盘）  
   `cd services/gateway && python -m venv .venv && . .venv/bin/activate && pip install -r requirements.txt && export DEV_SKIP_LARK=true AGENTS_BASE_URL=http://127.0.0.1:18080 && uvicorn app.main:app --host 0.0.0.0 --port 8000`
3. 终端 C：前端  
...
2. 页面自动 `POST /api/v1/auth/dev` 后拉取 **云空间目录** 列表；点 **「生成一条开发用总结」** 调 `POST /api/v1/dev/trigger-summary`，经 Mock Agents 在 DEV 下用占位直链。若已配置真云盘与 `ARTIFACTS_DRIVE_FOLDER_TOKEN`，**刷新列表**可在目标目录看到新文档（延迟取决于飞书与 Agents）。
